...
Examples:
  ...
  # Upload with custom thread count and manifest output
  %(prog)s --s3-bucket my-bucket --s3-prefix job-attachments --paths /data --threads 8 --manifest-output manifest.json
        """,
    )
    ...
    parser.add_argument(
        "--manifest-output",
        help="Path to save the upload manifest JSON file (default: upload_manifest.json)",
    )
    parser.add_argument(
        "--profile",
...
def generate_manifest(
    ...
    """
    Generate and save JSON manifest of uploaded files.

    Args:
        files: List of file info dicts (path, size, mtime, hash)
        output_path: Optional path to save manifest (default: upload_manifest.json)

    Returns:
        Path to the saved manifest file
    """
    if output_path is None:
        output_path = "upload_manifest.json"
    ...
    manifest = {
        ...
        "total_size": sum(f["size"] for f in manifest_entries),
...
def main() -> int:
    ...
    try:
        ...
        print(f"\n[2/5] Collecting files from {len(args.paths)} path(s)...")
