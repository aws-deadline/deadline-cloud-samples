...
def main():
    ...
    p.add_argument("--frames-dir", required=True,
                   help="Directory holding the rendered frame sequence, linked as "
                        "the Version's sg_path_to_frames.")
    args = p.parse_args()
    ...
    version = sg.create("Version", {
        ...
        "sg_path_to_frames": os.path.abspath(args.frames_dir),
    })
    ...
    # Print the Version URL from the connected client's base URL (not the secret
    # dict), so no secret-derived value is logged.
    print(f"Version: {sg.base_url.rstrip('/')}/detail/Version/{version['id']}")
