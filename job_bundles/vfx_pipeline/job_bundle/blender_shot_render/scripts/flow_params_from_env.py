...
(via a launcher, a "set project" script, and so on). This hook reads those
...
# Parameters the template constrains to uppercase TRUE/FALSE; normalize case so a
# lowercase FLOW_PUBLISH=false still matches the template's allowedValues.
BOOL_PARAMETERS = {"EnableFlowPublish"}
...
def main():
    ...
    for env_name, (param_name, required) in ENV_TO_PARAMETER.items():
        ...
        elif param_name in BOOL_PARAMETERS:
