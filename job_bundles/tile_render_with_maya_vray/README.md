...
```
        ...
        if any(v is not None for v in region):
            ...
            # Set the output size to be tile sized
            # maya.cmds.setAttr("vraySettings.width", region_maxX - region_minX)
            # maya.cmds.setAttr("vraySettings.height", region_maxY - region_minY)
