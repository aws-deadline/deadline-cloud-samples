...
sed -i "s|+ VRayForMaya2027rhel8 0.9 ../../maya_vray|+ VRayForMaya2027rhel8 0.9 $MAYA_VRAY_MAYA_ROOT|" $PREFIX/usr/autodesk/modules/maya/$MAYA_VERSION/VRayForMaya.module
...
if [ -n "\${MAYA_VRAY_${MAYA_VERSION}_SAVED_LD_LIBRARY_PATH+x}" ]; then
    if [ -z "\$MAYA_VRAY_${MAYA_VERSION}_SAVED_LD_LIBRARY_PATH" ]; then
        unset LD_LIBRARY_PATH
    else
        export LD_LIBRARY_PATH="\$MAYA_VRAY_${MAYA_VERSION}_SAVED_LD_LIBRARY_PATH"
    fi
    unset MAYA_VRAY_${MAYA_VERSION}_SAVED_LD_LIBRARY_PATH
fi
EOF