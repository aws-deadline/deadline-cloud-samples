...
if [ -z "${DEADLINE_JA_S3_BUCKET:-}" ] || [ -z "${BLENDER_VERSION:-}" ]; then
    echo "Simple Plugins: Skipping — DEADLINE_JA_S3_BUCKET or BLENDER_VERSION not set." >&2
...
if aws s3 ls "$_SP_GENERIC_SRC" >/dev/null 2>&1; then
    ...
    echo "Simple Plugins: Downloading generic plugins from $_SP_GENERIC_SRC" >&2
...
if aws s3 ls "$_SP_DCC_SRC" >/dev/null 2>&1; then
    echo "Simple Plugins: Downloading Blender plugins from $_SP_DCC_SRC" >&2
...
if [ -d "$_SP_PLUGIN_DIR" ] && [ "$(ls -A "$_SP_PLUGIN_DIR" 2>/dev/null)" ]; then
    ...
    echo "Simple Plugins: BLENDER_USER_SCRIPTS=$_SP_PLUGIN_DIR" >&2
...
"
        echo "Simple Plugins: Found addon package: $_name" >&2
    done
    for _entry in "$_SP_ADDONS_DIR"/*.py; do
        ...
        _SP_ADDON_LIST="${_SP_ADDON_LIST}    \"${_name}\",
"
        echo "Simple Plugins: Found addon script: $_name" >&2
    done
...
_auto_enable()
PYEOF
        echo "Simple Plugins: Generated auto-enable startup script for $(echo "$_SP_ADDON_LIST" | wc -l) addon(s)" >&2
    fi

    unset _SP_ADDONS_DIR _SP_STARTUP_DIR _SP_ADDON_LIST _entry _name
else
    echo "Simple Plugins: No Blender plugins found, skipping." >&2
fi
