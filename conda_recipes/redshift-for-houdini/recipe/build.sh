...
mv $SRC_DIR/redshift $REDSHIFT_LOCATION
...
echo "Not empty???"
        if [[ ! \$HOUDINI_VERSION =~ [0-9]+\.[0-9]+\.[0-9]+ ]]; then
                ...
                MAX=\$(find \$RED_LOCATION -maxdepth 1 -name '\$HOUDINI_VERSION\.[1-9]*' |
...
export HOUDINI_PATH="\$RED_LOCATION/redshift4houdini/\${EXACT_HOUDINI_VERSION};\${HOUDINI_PATH-&}"
EOL
