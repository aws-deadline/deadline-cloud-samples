...
# and exit 0.
#
# The delete below is tidying, not a guarantee, which is why it tolerates failure: Move-Item -Force
# replaces $dest whether or not this succeeded, and a $dest held open by another process surfaces as a
# clear error from Move-Item rather than as a stale script being run.
