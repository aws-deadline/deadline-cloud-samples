...
- A fleet IAM role with `s3:GetObject` on the installers, plus `s3:GetObject`, `s3:PutObject`,
  ...
  rather than the prefix, and without it S3 answers a missing key with `403` instead of `404`, so the
  ordinary "no cached payload yet" case is indistinguishable from a denial. `s3:DeleteObject` is
  required so the install token is released after a cold install. Without it, later workers wait out the
