...
# anything. Staleness does not need to cover a whole install: Update-InstallToken re-stamps the token
# at each long step, so the threshold measures liveness rather than elapsed install time.
$TOKEN_STALE_MIN = 40
...
function Test-S3Object([string]$uri) {
    ...
    foreach ($attempt in 1..3) {
        ...
        cmd /c exit 0
        if ($err -match '404|Not Found') { return $false }
        if ($attempt -lt 3) {
...
# Before anything writes a scratch file or touches S3. Both the token body and the captured stderr
# that drives the claim logic live here, so this has to be hardened earlier than any other directory.
Protect-Dir $SCRATCH $null
# Fail loudly rather than degrade. Every stderr-capturing helper redirects into $SCRATCH, and a cmd
# redirect into a missing directory returns non-zero with no output. In Get-InstallClaim that reads as
# an unrecognized error and every worker concludes the token is held, so a whole cold fleet waits for
# a zip nobody is building. This assert turns a future reordering into an obvious failure.
...
function Export-InstallerState([switch]$SkipHostLicensing) {
    ...
    foreach ($s in $ServiceNames) {
        ...
        if ($svc) {
            ...
            } | ConvertTo-Json | Out-File "$graftNew\services\$($s -replace '[^A-Za-z0-9]','_').json" -Encoding utf8
...
try {
    ...
    Install-3dsMax
    Update-InstallToken      # the install is the longest step; prove liveness before continuing
    Install-Pencil
